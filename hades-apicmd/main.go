...
func init() {
	...
	flag.StringVar(&cmdAddr,     "addr", env("HADES_API_ADDR",""), "the api addr to access such as 127.0.0.1:9001 or form env(HADES_API_ADDR)")
	flag.StringVar(&cmdToken,   "token", env("HADES_API_TOKEN",""), "the token to auth, or from env(HADES_API_TOKEN)")
...
func doreq( a apiService)(error,string) {
       b, err := json.Marshal(a)
        ...
        client := &http.Client{}
        req, err := http.NewRequest(cmdMethod, cmdAddr,body)
        if err != nil {
                return err, ""
        }
        req.Header.Set("Content-Type", "application/json;charset=UTF-8")
        req.Header.Set("token", cmdToken)
        req.Header.Set("domain", cmdDomain)
...
func main() {
	...
	if !strings.HasSuffix(cmdAddr, "/hades"){
		cmdAddr = "http://"+ cmdAddr + "/hades"
	}
        if cmdList{
		cmdMethod = "GET"
		cmdApiList()
