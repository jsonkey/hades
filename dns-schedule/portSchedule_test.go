...
func TestReadConfig(t *testing.T) {
    configPath := "/etc/hades/hades-schedule.conf"
...
func TestLoopReportData(t *testing.T) {
    if _, _, _, err := loopReportData(); err != nil {
        t.Error(err.Error())
    }
}
...
func TestLoopHeartBeat(t *testing.T) {
    allocationdata, _, _, _ := loopReportData()
    _, _, _, heartbeat_error := loopHeartBeat(allocationdata)
...
func TestScheduleWaitPool(t *testing.T) {
    allocationdata, initdata, total_num, _ := loopReportData()
    active_agents, inactive_agents, new_agents, _ := loopHeartBeat(allocationdata)
